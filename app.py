...
HISTORY_FILE = os.getenv('HISTORY_FILE', '/mnt/data/post_history.json')
UPLOAD_DIR = os.getenv('UPLOAD_DIR', '/mnt/data/uploads')
...
def _save_settings(data: dict):
    try:
        _Path(SETTINGS_FILE).parent.mkdir(parents=True, exist_ok=True)
...
def _save_history(hashes):
    try:
        _Path(HISTORY_FILE).parent.mkdir(parents=True, exist_ok=True)
...
<body>
  <div class="container">
    ...
    <div id="tab-posting" class="tab card" style="display:none">
      ...
      <div class="row" style="margin-top:8px">
        <textarea id="ai_prompt" placeholder="Prompt để AI viết bài (tùy chọn)..."></textarea>
        <div class="row">
    ...
    <div id="tab-settings" class="tab card" style="display:none">
      <h3>Cài đặt</h3>
      <div class="muted">Mỗi Page: <b>Từ khoá</b> + <b>Link truy cập</b>. Tên Page sẽ hiển thị ở đây.</div>
  ...
  async function loadSettings(){
    ...
    try{
      ...
      const rows = (d.data||[]).map(s => (
        ...
        '<div style=\"min-width:200px\"><b>'+s.name+'</b></div>' +
        '<input type=\"text\" class=\"set-keyword\" data-id=\"'+s.id+'\" placeholder=\"Từ khoá (ví dụ: MB66)\" value=\"'+(s.keyword||'')+'\" style=\"flex:1\">' +
        '<input type=\"text\" class=\"set-link\" data-id=\"'+s.id+'\" placeholder=\"Link truy cập (ví dụ: https://...)\" value=\"'+(s.link||'')+'\" style=\"flex:1\">' +
        '</div>'
      )).join('');
  ...
  $('#btn_settings_save')?.addEventListener('click', async ()=>{
    ...
    $all('.set-keyword').forEach(inp => {
      ...
      items.push({id, keyword: inp.value||'', link});
    });
...
# ------------------------ Settings (keyword/link + page name) ------------------------
@app.route("/api/settings/get")
def api_settings_get():
    data = _load_settings()
    pages = []
    for pid, token in PAGE_TOKENS.items():
        # fetch real page name
        try:
            info = fb_get(pid, {"access_token": token, "fields": "name"})
...
def api_settings_save():
    ...
    for it in items:
        pid = it.get("id")
        if not pid: continue
        # accept legacy ai_key as keyword if present
...
# ------------------------ AI Generator: spintax + icons + hashtags + contact + uniqueness ------------------------

_SPX = re.compile(r"\{([^{}]+)\}")

def _spin(text: str) -> str:
    def choose(opts): return (opts.split('|'))[hash(text+str(time.time_ns())) % len(opts.split('|'))]
    s = text
    while True:
        m = _SPX.search(s)
        if not m: break
...
def api_ai_generate():
    ...
    data = _load_settings()
    st = (data.get(page_id) or {}) if page_id else {}
...
def api_upload():
    try:
        ...
        _Path(UPLOAD_DIR).mkdir(parents=True, exist_ok=True)
        fn = f.filename or "media.bin"
        safe = re.sub(r"[^0-9A-Za-z_.-]", "_", fn)
        p = _Path(UPLOAD_DIR) / safe
        f.save(str(p))
...
# (Optional) serve uploaded files for verification in dev
